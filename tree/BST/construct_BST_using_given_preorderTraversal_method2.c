    ...
    if( key > min && key < max )
    {
        ...
        *preIndex = *preIndex + 1;
        ...
        if (*preIndex < size)
        {
            ...
            printf("Lets construct %d left subtree with preindex = %d\n", key, *preIndex);
            root->left = constructTreeUtil( pre, preIndex, pre[*preIndex],
                                        min, key, size );

            // All nodes which are in range {key..max} will go in right
            // subtree, and first such node will be root of right subtree.
            printf("Lets construct %d right subtree with preindex = %d\n", key, *preIndex);
            root->right = constructTreeUtil( pre, preIndex, pre[*preIndex],
                                         key, max, size );
        }
    }
    if(!root)
        printf("****Key %d does not lie b/w %d and %d****\n", key, min, max);
    else
        printf("\nExecution of %d over, Returning from %d\n",key, key);
    return root;
}
